...
#include 'mafdecls.fh'
        ...
        if(myid.eq.0) then
            ...
            write(unit=RESTARTFILE,fmt=1001)alpha(1:iter)
            write(unit=RESTARTFILE,fmt='(a)')' BETA COEFFICIENTS -- off-diagonal T matrix elements'
            write(unit=RESTARTFILE,fmt=1001)beta(0:iter)
            if(orthog) then
...
#include 'global.fh'
    ...
    subroutine load_restartinfo(reloadall)
        ...
        reloadall = i==nload
        read(unit=RESTARTFILE,fmt='(a75)')commentline
        read(unit=RESTARTFILE,fmt=1000)(alpha(i),i=1,nload)
        searchterm=' BETA CO'
        do 
            read(unit=RESTARTFILE,fmt=1001,end=10,ERR=10)currterm
            if(currterm==searchterm) exit
        end do
        read(unit=RESTARTFILE,fmt=1000)(beta(i),i=0,nload)
        if(orthog) then
            searchterm=' OMEGA(1'
            do 
                read(unit=RESTARTFILE,fmt=1001,end=10,ERR=10)currterm
